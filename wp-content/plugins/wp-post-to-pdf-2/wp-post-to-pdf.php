...
global $authordata, $id, $blog_id;
define('WPPT0PDF_NAME', 'WP Post to PDF');
define('WPPT0PDF_SNAME', 'wpptopdf');
define('WPPT0PDF_PATH', WP_PLUGIN_DIR . '/wp-post-to-pdf-2');
define('WPPT0PDF_URL', WP_PLUGIN_URL . '/wp-post-to-pdf-2');
    ...
    class wpptopdf
    {
        ...
        function generate_pdf_file($id, $forceDownload = false)
        {
           ...
           //     $post->post_content = do_shortcode($post->post_content);
            ...
            $pdf->SetHeaderData(PDF_HEADER_LOGO, $logowidth, '', html_entity_decode(get_bloginfo('description') . "\n" . get_bloginfo('siteurl')));
            ...
            $the_user = get_userdata($post->post_author);
			...
			$ac_author_lastname = $the_user->last_name;
			$ac_author_firstinit = substr($the_user->first_name,0,1);
			$ac_author_firstname = $the_user->first_name;
			//$ac_author_lastname = $user_info->last_name;
			//$ac_author_firstinit = substr($user_info->first_name,0,1);
			//$ac_author_firstname = $user_info->first_name;
            $ac_post_title = $post->post_title;
			...
			while ($n <= (count($ac_author_lastnames)-1)) {
				...
				$ama_block .= ". ".$ac_post_title.". <em>".$ac_blogname."</em>. ".$ac_post_dateY.". Available at: ".$ac_post_permalink.". Accessed ".$ac_access_dateFjY.".";
			 ...
			 while ($n <= (count($ac_author_lastnames)-1)) {
			 	...
			 	$apa_block .= ". (".$ac_post_dateY."). <em>".$ac_post_title."</em>. Retrieved ".$ac_access_dateFjY.", from ".$ac_blogname." Web site: ".$ac_post_permalink;
			 ...
			 while ($n <= (count($ac_author_lastnames)-1)) {
			   ...
			   while ($n <= (count($ac_author_lastnames)-1)) {
			   	...
			   	$harvard_block .= " ".$ac_post_dateY.", <em>".$ac_post_title."</em>, ".$ac_blogname.". Retrieved ".$ac_access_dateFjY.", from ";
            	$harvard_url = "&lt;".$ac_post_permalink."&gt;";
			 ...
			 while ($n <= (count($ac_author_lastnames)-1)) {
			 	...
			 	$mla_block .= ". \"".$ac_post_title.".\" ".$ac_post_datejfY.". <u>".$ac_blogname."</u>. Accessed ".$ac_access_datejfY.".";
            	$mla_url = "&lt;".$ac_post_permalink."&gt;";
            
            
            	// $pdf_content = $post->post_content;
            	$pdf_content = apply_filters('the_content', $post->post_content);
            	// $output = preg_replace_callback("/(&#[0-9]+;)/", function($m) { return mb_convert_encoding($m[1], "UTF-8", "HTML-ENTITIES"); }, $pdf_content);
            	// $pdf_content = get_the_content();
            	/***********************************************/
            	/* OUTPUT TO TEMPLATE */
			 $n=0;
			 while ($n <= (count($ac_author_lastnames)-1)) {
			 	if ($n === 0) {
			 		$author_block = $ac_author_firstnames[$n]." ".$ac_author_lastnames[$n];
			 	} elseif ($n === (count($ac_author_lastnames)-1)) {
			 		$author_block .= " and ".$ac_author_firstnames[$n]." ".$ac_author_lastnames[$n];
			 	} else {
			 		$author_block .= ", ".$ac_author_firstnames[$n]." ".$ac_author_lastnames[$n];
			 	}
			 	$n++;
            ...
            $html = '<style type="text/css">article p { margin-bottom: 20px; margin-top: 20px;} .sharing {display:none;} .wp-caption-text {background-color:#ffffff;}</style>';
            $html .= '<p style="color:#777777;"><a href="' . html_entity_decode($ac_post_permalink, ENT_QUOTES, 'UTF-8') . '">'. $ac_post_permalink .'</a></p>';
            $html .= '<h1 style="font-weight:normal">' . html_entity_decode($ac_post_title, ENT_QUOTES, 'UTF-8') . '</h1>';
            $html .= '<p style="color:#777777;">' . html_entity_decode($post->post_date, ENT_QUOTES, 'UTF-8') . '</p>';
            $html .= '<p style="color:#777777;">By ' . $author_block . '</p>';
            $html .= html_entity_decode($pdf_content, ENT_QUOTES, 'UTF-8');
            $html .= '<p><strong>AMA citation</strong><br/>';
            $html .= html_entity_decode($ama_block, ENT_QUOTES, 'UTF-8');
            $html .= '</p>';
            $html .= '<p><strong>APA citation</strong><br/>';
            $html .= html_entity_decode($apa_block, ENT_QUOTES, 'UTF-8');
            $html .= '</p>';
            $html .= '<p><strong>Chicago citation</strong><br/>';
            $html .= html_entity_decode($chicago_block, ENT_QUOTES, 'UTF-8');
            $html .= '</p>';
            $html .= '<p><strong>Harvard citation</strong><br/>';
            $html .= html_entity_decode($harvard_block, ENT_QUOTES, 'UTF-8');
            $html .= $harvard_url;
            $html .= '</p>';
            $html .= '<p><strong>MLA citation</strong><br/>';
            $html .= html_entity_decode($mla_block, ENT_QUOTES, 'UTF-8');
            $html .= $mla_url;
            $html .= '</p>';
